...
mix1 = pd.read_csv("data_figure.csv")
mix_min_max1 = pd.read_csv("data_figure_minmax.csv")
mix_min_max1['min'] = mix_min_max1['min'] * 100
mix_min_max1['max'] = mix_min_max1['max'] * 100
cat_order = ['BIO', 'OIL', 'GAS', 'WAT', 'SUN', 'WIN']
...
if scenario == 'Historical':
    ...
    mix = mix1[mix1['Scenario'] != 'Historical']
    mix_min_max1['Label'] = mix_min_max1['min'].round(1).astype(str) + '% - ' + mix_min_max1['max'].round(1).astype(
        str) + '%'
    mix_min_max = mix_min_max1[mix_min_max1['Scenario'] != "Historical"]
...
if scenario == 'Historical':
    (ggsave(g, "figure_Historical.png", width=15.5 / 2.5, height=12 / 2.5))
else:
    (ggsave(g, "figure_Baseline.png", width=15.5 / 2.5, height=12 / 2.5))
