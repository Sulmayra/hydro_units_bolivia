...
"************************* OutputPower _*****************************"
df1 = pd.read_csv('./OutputPower/OutputPower_HydroUnits_2010.csv', index_col=0)
df2 = pd.read_csv('./OutputPower/OutputPower_Total.csv', index_col=0)
colores = ['#F1E9E5', '#B4B897','#368B85','#464660','#B42B51','#4A47A3','#413C69','#709FB0','#A7C5EB']
df1.plot(kind="bar",width=0.8,figsize=(14, 2.5), color= colores, fontsize=11, alpha=1)
plt.plot(df2, marker='o', linestyle='--', color='#2C2E43', label = 'Total')
...
for z in ['1980','1990','2000']:
    

    df1 = pd.read_csv('./OutputPower/OutputPower_HydroUnits_'+z+'.csv', index_col=0)
    df2 = pd.read_csv('./OutputPower/OutputPower_Total.csv', index_col=0)
    colores = ['#F1E9E5', '#B4B897','#368B85','#464660','#4A47A3','#709FB0','#B42B51','#413C69','#B42B51']
    df1.plot(kind="bar",width=0.8,figsize=(14, 2.5), color= colores, fontsize=11, alpha=1)
    plt.plot(df2, marker='o', linestyle='--', color='#2C2E43', label = 'Total')
    plt.legend(bbox_to_anchor=(1, 1.0), loc='upper left')
    plt.xticks(rotation=0)
