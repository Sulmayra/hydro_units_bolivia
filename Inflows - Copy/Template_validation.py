...
Hydro = ['VALLE DE ZONGO - CAHUA','VALLE DE ZONGO - CHURURAQUI','VALLE DE ZONGO - HARCA', 'VALLE DE ZONGO - HUAJI']
colores = ['#F1E9E5', '#B4B897','#368B85','#464660','#FACE7F','#4A47A3','#413C69','#709FB0','#B42B51']

# fig= plt.tight_layout()
# fig = plt.figure(figsize=(16,15))

for h in Hydro:
    df = pd.read_csv('./Inflows_validation/'+h+'.csv', index_col=0)
    for i in [1,2,3,4]:
        plt.subplot(2,3,str(i))
        for f in df:            
            plt.plot(str(f), marker="o",  alpha=0.8) # title='Installed capacity per zone (the horizontal lines indicate the peak demand)'  
            plt.xlabel('Date')
            plt.ylabel('Flow [m3/s]')
    
    
#     df.plot(kind="line", marker="o",color = colores, stacked=False, alpha=0.8, legend='reverse', fontsize=12,  
#     title='Inflows - Zone: ' + str(h) ) # title='Installed capacity per zone (the horizontal lines indicate the peak demand)'  
#     plt.xlabel('Date')
#     plt.ylabel('Flow [m3/s]')
